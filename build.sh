...
then
  NO_MENUCONFIG=1
  MAKE_STATEMENT="$MAKE_STATEMENT KCONFIG_CONFIG=./arch/arm64/configs/$DEFCONFIG"
fi
...
then
  ...
  echo -e "> Starting kernel compilation using $DEFCONFIG file directly...\n"
else
  if [ -f ".config" ]
  then    
    echo -e "\033[0;32m> Config file already exists\033[0;0m\n"
  else
    echo -e "\033[0;31m> Config file not found, copying $DEFCONFIG as .config...\033[0;0m\n" 
...
KBUILD_LOUP_CFLAGS="-Wno-unused-variable -Wno-parentheses -Wno-memset-elt-size -Wno-bool-operation -Wno-tautological-compare -Wno-duplicate-decl-specifier -Wno-misleading-indentation -Wno-bool-compare -mtune=cortex-a53 -march=armv8-a+crc+simd+crypto -mcpu=cortex-a53 -O2" 
